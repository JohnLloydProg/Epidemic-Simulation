...
def load_graph() -> Graph:
    ...
    try:
        ...
        for i in range(len(nodes)):
            ...
            singleton_instance.add_node(node)
        
        print("Nodes: ", [node.id for node in singleton_instance.nodes])
        edges = pd.read_excel(f"{city_path}/edges.xlsx", index_col=0)
        
        for i in range(len(edges)):
            edge_xl = edges.iloc[i]
            singleton_instance.add_edge(random.randint(10, 40), int(edge_xl['Node 1'] - 1), int(edge_xl['Node 2'] - 1))
        ...
        for i in range(len(regions)):
            ...
            singleton_instance.add_region(node_ids, int(region_xl['Alloted Residential Units']), int(region_xl['Alloted Business Units']))
